...
// Book file: public/books/gm2600.bin
// Spec: 16-byte entries sorted by Zobrist key.
//   key[8B BE uint64] | move[2B BE uint16] | weight[2B BE uint16] | learn[4B BE uint32]

import { Chess } from 'chess.js';

// ── Standard Polyglot Random64 table (781 values) ────────────────────────────
// Source: https://raw.githubusercontent.com/niklasf/python-chess/master/chess/polyglot.py
// Starting-position verification key: 0x463B96181691FC9C
...
// ── Piece type → Random64 index (0-based) ────────────────────────────────────
const PIECE_TYPE_IDX: Record<string, number> = { p: 0, n: 1, b: 2, r: 3, q: 4, k: 5 };

// ── Compute Polyglot Zobrist key from FEN ─────────────────────────────────────
export function polyglotKey(fen: string): bigint {
  ...
  // Pieces: board[row][col], row 0 = rank 8, col 0 = file a
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      const piece = board[row][col];
      if (!piece) continue;
      const rank0 = 7 - row;              // 0 = rank 1
      const sq = rank0 * 8 + col;         // a1=0 … h8=63
      const typeIdx = PIECE_TYPE_IDX[piece.type];
      // Polyglot spec: White=1, Black=0 (not the other way around)
      const colorIdx = piece.color === 'w' ? 1 : 0;
      const pieceIdx = typeIdx * 2 + colorIdx; // 1=WP,0=BP,3=WN,2=BN,…
      key ^= Random64[pieceIdx * 64 + sq];
    }
  }

  // Turn
  if (sideToMove === 'b') key ^= Random64[780];
  ...
  if (epStr && epStr !== '-') {
    const epFile = epStr.charCodeAt(0) - 97; // a=0…h=7
    if (canCaptureEnPassant(board, epFile, sideToMove)) {
...
function canCaptureEnPassant(board: (null | { type: string; color: string })[][], epFile: number, side: 'w' | 'b'): boolean {
  // White captures to rank 6 (e.g. e6); White's pawn is on rank 5 → board row 3
  // Black captures to rank 3 (e.g. e3); Black's pawn is on rank 4 → board row 4
  const pawnRow = side === 'w' ? 3 : 4;
  for (const f of [epFile - 1, epFile + 1]) {
    if (f < 0 || f > 7) continue;
...
// ── Move decoding ─────────────────────────────────────────────────────────────
const SQ_FILE = (sq: number) => String.fromCharCode(97 + (sq % 8));
const SQ_RANK = (sq: number) => String(Math.floor(sq / 8) + 1);
const PROMO = ['', 'n', 'b', 'r', 'q'];

function polyMoveToUci(moveWord: number): string {
  const toSq   = moveWord & 0x3F;
  const fromSq = (moveWord >> 6) & 0x3F;
  const promo  = (moveWord >> 12) & 0x7;

  let from = SQ_FILE(fromSq) + SQ_RANK(fromSq);
  let to   = SQ_FILE(toSq)   + SQ_RANK(toSq);

  // Polyglot castling: king moves to rook square → translate to king's destination
  if (fromSq === 4  && toSq === 7)  to = 'g1';
  if (fromSq === 4  && toSq === 0)  to = 'c1';
  if (fromSq === 60 && toSq === 63) to = 'g8';
  if (fromSq === 60 && toSq === 56) to = 'c8';

  return from + to + (promo > 0 ? PROMO[promo] : '');
}

// ── Book loading ──────────────────────────────────────────────────────────────
let _bookBuf: ArrayBuffer | null = null;
let _bookLoaded = false;
let _bookLoadPending: Promise<void> | null = null;

export async function ensureBookLoaded(): Promise<void> {
...
export function isBookAvailable(): boolean {
  return _bookBuf !== null;
}

// ── Binary search helpers ─────────────────────────────────────────────────────
function readKey(view: DataView, idx: number): bigint {
  const o = idx * 16;
  return (BigInt(view.getUint32(o, false)) << 32n) | BigInt(view.getUint32(o + 4, false));
}

function binarySearch(view: DataView, target: bigint): number {
  let lo = 0, hi = Math.floor(view.byteLength / 16) - 1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    const k = readKey(view, mid);
...
// ── Public API ────────────────────────────────────────────────────────────────
export async function getBookMoves(fen: string): Promise<string[]> {
  await ensureBookLoaded();
  if (!_bookBuf) return [];

  const view = new DataView(_bookBuf);
  const target = polyglotKey(fen);
  let idx = binarySearch(view, target);
  if (idx < 0) return [];

  // Scan back to first entry with this key
  while (idx > 0 && readKey(view, idx - 1) === target) idx--;

  const moves: string[] = [];
  const total = Math.floor(view.byteLength / 16);
  while (idx < total && readKey(view, idx) === target) {
    const moveWord = view.getUint16(idx * 16 + 8, false);
    moves.push(polyMoveToUci(moveWord));
    idx++;
  }
  return moves;
}
...
export async function isBookMove(fen: string, uci: string): Promise<boolean> {
  const moves = await getBookMoves(fen);
  return moves.includes(uci);
}
